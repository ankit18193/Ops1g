    ...
    () =>
      leads
        ...
        .filter(
          (l) =>
            !q ||
            `${l.name} ${l.phoneRaw} ${l.area} ${l.stage}`.toLowerCase().includes(q.toLowerCase()),
        )
    ...
    <div className="min-h-screen bg-background text-foreground p-3 space-y-3 pip-compact">
        ...
        <div className="grid grid-cols-4 gap-1">
          {(["new", "old", "future", "past"] as Bucket[]).map((b) => (
            <button
              ...
              onClick={() => setBucket(b)}
              className={cn(
                "rounded-md border px-1 py-1 text-[10px] capitalize",
                bucket === b
      ...
      <div className="grid grid-cols-1 gap-2">
        {filtered.slice(0, 8).map((l) => (
          <button
            key={l.ulid}
            onClick={() => setSelected(l.ulid)}
            className={cn(
              "text-left rounded-lg border p-2",
              lead?.ulid === l.ulid ? "border-primary bg-primary/5" : "border-border bg-card",
            )}
          >
            ...
            <div className="text-[10px] text-muted-foreground truncate">
              {l.area || "No area"} · {l.budget ? `₹${l.budget.toLocaleString()}` : "No budget"}
            </div>
          </button>
        ))}
        {filtered.length === 0 && (
          <div className="rounded-lg border border-border bg-card p-4 text-xs text-muted-foreground text-center">
            No {bucket} leads.
          </div>
        )}
      </div>
        ...
        <section className="rounded-xl border border-border bg-card p-3 space-y-3">
          <div>
            ...
            <div className="text-[11px] text-muted-foreground">
              {lead.phoneRaw || lead.phoneE164} · {lead.zone || "No zone"}
            </div>
          </div>
          <div className="flex gap-1 flex-wrap">
            ...
            <a
              href={`https://wa.me/${(lead.phoneE164 || lead.phoneRaw).replace(/\D/g, "")}`}
              target="_blank"
              rel="noreferrer"
              className="inline-flex h-8 items-center gap-1 rounded-md border border-border px-2 text-xs"
            >
          ...
          <div className="grid grid-cols-2 gap-1">
            {states.map((s) => (
              <button
                key={s}
                onClick={() => setLifecycleState(lead.ulid, s)}
                className={cn(
          ...
          <Button
            ...
            onClick={() => {
              if (note.trim()) {
                logActivity(lead.ulid, "note-added", note.trim());
                setNote("");
              }
            }}
          >
          ...
          <div className="space-y-1">
            ...
            {timeline.map((a) => (
              <div key={a.id} className="text-[10px] text-muted-foreground flex gap-1">
                <Clock className="h-3 w-3" /> {a.text}
              </div>
            ))}
            {timeline.length === 0 && (
              <div className="text-[10px] text-muted-foreground flex gap-1">
                <Archive className="h-3 w-3" /> No activity yet
              </div>
            )}
          </div>
        </section>
      ...
      <div className="text-[10px] text-muted-foreground flex items-center gap-1">
        ...
        <UserCheck className="h-3 w-3" /> Edits sync back to the main dashboard.
      </div>
    </div>
  );
}
