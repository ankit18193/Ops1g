...
export function BulkPasteImport() {
  ...
  const onParse = () => {
    ...
    if (!drafts.length) {
      toast.error("No leads detected in pasted text");
      return;
    }
    ...
    <div className="space-y-3">
      <div className="rounded-xl border border-border bg-card p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <Button size="sm" onClick={onParse} disabled={!raw.trim()}>
            Parse all
          </Button>
          {rows.length > 0 && (
            <Button size="sm" variant="default" className="gap-1" onClick={onImport}>
              <ShieldCheck className="h-3.5 w-3.5" /> Import{" "}
              {rows.filter((r) => r.selected && r.match.type !== "exact").length} leads
            </Button>
          )}
        </div>
      </div>
            ...
            <tbody className="divide-y divide-border">
              {rows.map((r, i) => (
                <tr key={i} className="hover:bg-muted/20">
                  ...
                  <td className="px-3 py-2">
                    {r.draft.name || <span className="text-muted-foreground">—</span>}
                  </td>
                  <td className="px-3 py-2 font-mono text-xs">{r.draft.phone || "—"}</td>
                  <td className="px-3 py-2 text-xs">
                    {r.draft.location || "—"}{" "}
                    {r.draft.zone && (
                      <span className="text-muted-foreground">· {r.draft.zone}</span>
                    )}
                  </td>
                  <td className="px-3 py-2">
                    <Badge
                      variant="outline"
                      className={`text-[10px] capitalize ${matchColor(r.match.type)}`}
                    >
