...
export function PostVisitGate({ lead }: { lead: Lead }) {
  ...
  const lastVisit = useMemo(
    () =>
      tours
        .filter((t) => t.leadId === lead.id && t.status === "completed")
  ...
  const submitTimeline = () => {
    if (!decisionBy) {
      toast.error("Pick a date or reason");
      return;
    }
    addCommitment({
      leadId: lead.id,
  ...
  const submitReason = () => {
    if (reason.trim().length < 5) {
      toast.error("Describe why no decision timeline");
      return;
    }
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="text-xs">
          <div className="font-bold text-destructive">
            48h post-visit gate ·{" "}
            {mounted
              ? formatDistanceToNow(new Date(lastVisit!.scheduledAt), { addSuffix: true })
              : "recently"}
          </div>
      ...
      <div className="space-y-2 rounded-md bg-card p-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <Input
              className="h-8 text-xs"
              placeholder='e.g. "I will decide by Thursday"'
              value={exactWords}
