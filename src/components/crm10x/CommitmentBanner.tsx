...
export function CommitmentBanner({ lead }: { lead: Lead }) {
  const commitment = useCRM10x(
    (s) => s.commitments.filter((c) => c.leadId === lead.id && c.status === "pending")[0],
  );
    ...
    <div className={`mx-5 mt-3 rounded-lg border p-3 flex items-start gap-2 ${tone}`}>
      ...
      <div className="flex gap-1">
        <Button
          ...
          onClick={() => {
            resolve(commitment.id, "kept");
            toast.success("Marked kept");
          }}
        >
