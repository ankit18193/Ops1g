      ...
      <CommandList>
        ...
        <CommandGroup heading="Navigate">
          ...
          <CommandItem onSelect={() => go("/today")}>
            <Sun className="mr-2 h-4 w-4" /> Today (Do next)
          </CommandItem>
          <CommandItem onSelect={() => go("/leads")}>
            <Target className="mr-2 h-4 w-4" /> Leads
          </CommandItem>
          <CommandItem onSelect={() => go("/tours")}>
            <CalendarPlus className="mr-2 h-4 w-4" /> Tours
          </CommandItem>
          <CommandItem onSelect={() => go("/follow-ups")}>
            <ClipboardList className="mr-2 h-4 w-4" /> Follow-ups
          </CommandItem>
          <CommandItem onSelect={() => go("/handoffs")}>
            <MessageSquare className="mr-2 h-4 w-4" /> Handoffs
          </CommandItem>
          <CommandItem onSelect={() => go("/sequences")}>
            <Zap className="mr-2 h-4 w-4" /> Sequences
          </CommandItem>
          <CommandItem onSelect={() => go("/revival")}>
            <Sparkles className="mr-2 h-4 w-4" /> Revival queue
          </CommandItem>
          <CommandItem onSelect={() => go("/heatmap")}>
            <MapPin className="mr-2 h-4 w-4" /> Demand heatmap
          </CommandItem>
          <CommandItem onSelect={() => go("/revenue")}>
            <IndianRupee className="mr-2 h-4 w-4" /> Revenue
          </CommandItem>
        ...
        <CommandGroup heading="Leads">
          {leads.slice(0, 30).map((l) => (
            <CommandItem
              key={l.id}
              value={`${l.name} ${l.phone} ${l.preferredArea}`}
              onSelect={() => openLead(l.id)}
            >
              <Target className="mr-2 h-4 w-4" />
              <span className="font-medium">{l.name}</span>
              <span className="ml-2 text-xs text-muted-foreground">
                {l.phone} · {l.preferredArea}
              </span>
            </CommandItem>
          ))}
        </CommandGroup>
        <CommandGroup heading="Quick actions on selected lead">
          ...
          {leads.slice(0, 5).map((l) => (
            <CommandItem
              key={`wa-${l.id}`}
              value={`whatsapp ${l.name}`}
              onSelect={() => {
                sendMessage(l.id, "WhatsApp template sent");
                setOpen(false);
              }}
            >
