...
export const Route = createFileRoute("/supply-hub/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "200+ verified PGs with persona, scarcity, commute and value intelligence.",
      },
    ],
  }),
...
function SupplyHubHome() {
  ...
  const areas = useMemo(
    () => [
      "All",
      ...Array.from(new Set(PGS.map((p) => p.area)))
        .filter(Boolean)
        .sort(),
    ],
    [],
  ...
  const results = useMemo(() => {
    let hits = q.trim()
      ? searchPGs(q, 60)
      : PGS.slice(0, 60).map((pg) => ({ pg, score: 1, matched: [] as string[] }));
      ...
      <div className="space-y-6">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="text-[10px] uppercase tracking-wider text-accent font-semibold mb-1">
              Internal · HR / Flow Ops / TCM
            </div>
            <h1 className="font-display text-2xl font-semibold tracking-tight">
              Inventory Supply Hub
            </h1>
            <p className="text-sm text-muted-foreground mt-1">
              Verified PG network — search by name, area, landmark, or company. Every tile is
              computed live: persona, scarcity, freshness, value.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div
              key={s.label}
              className={cn("rounded-lg border bg-card p-4", s.accent && "border-accent/40")}
            >
        ...
        <div className="rounded-lg border bg-card p-3 flex flex-wrap items-center gap-3">
          ...
          <Pill
            label="Tier"
            value={tier}
            options={TIERS as readonly string[]}
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <h3 className="mt-0.5 font-semibold truncate group-hover:text-accent">
                      {pg.name}
                    </h3>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-1.5">
                  ...
                  {!sc.hot && sc.level !== "AVAILABLE" && (
                    <span className="rounded-md border border-amber-400/40 bg-amber-400/10 text-amber-300 px-1.5 py-0.5 text-[10px] font-medium">
                      {sc.level}
                    </span>
                  )}
                  {fr.isFresh && fr.changeKind && (
                    <span className="rounded-md border border-emerald-400/40 bg-emerald-400/10 text-emerald-300 px-1.5 py-0.5 text-[10px] font-medium">
                      {fr.changeKind}
                    </span>
                  )}
                ...
                {matched.length > 0 && (
                  <div className="mt-2 text-[10px] text-muted-foreground truncate">
                    Matched: {matched.slice(0, 2).join(" · ")}
                  </div>
                )}
              </Link>
            );
