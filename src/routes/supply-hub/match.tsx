      ...
      <div className="space-y-5">
        ...
        <header>
          ...
          <h1 className="font-display text-2xl font-semibold tracking-tight">
            Find the best PG for a lead
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Real distance + actual bed prices · never fabricated. Hard-disqualifies on gender /
            over-budget / occupancy not offered.
          </p>
        </header>
        ...
        >
          ...
          <Field label="Wants area / landmark / company">
            <input
              ...
              placeholder="e.g. Manyata, Christ, Koramangala"
              className="input"
            />
          </Field>
          <Field label="Gender">
            <select
              value={lead.gender}
              onChange={(e) => setLead({ ...lead, gender: e.target.value as Lead["gender"] })}
              className="input"
            >
              <option>Any</option>
              <option>Boys</option>
          ...
          <Field label="Occupancy">
            <select
              ...
              onChange={(e) => setLead({ ...lead, occupancy: e.target.value as Lead["occupancy"] })}
              className="input"
            >
              <option>Any</option>
          ...
          <Field label="Audience">
            <select
              value={lead.audience}
              onChange={(e) => setLead({ ...lead, audience: e.target.value as Lead["audience"] })}
              className="input"
            >
              <option>Both</option>
          ...
          <div className="flex items-end">
            <button
              type="submit"
              className="inline-flex items-center gap-1 rounded-md bg-accent px-4 py-2 text-sm font-medium text-accent-foreground hover:opacity-90"
            >
        ...
        {submitted && (
          <div className="space-y-3">
            ...
            <div className="rounded-lg border bg-card p-3 grid gap-2 md:grid-cols-3 text-xs">
              ...
              <Cell
                k="Nearest Supply Hub PG"
                v={results.find((r) => r.commuteKm !== null)?.pg.name ?? "Need known area/landmark"}
              />
              <Cell
                k="Lead distance"
                v={
                  results.find((r) => r.commuteKm !== null)?.commuteKm !== null &&
                  results.find((r) => r.commuteKm !== null)
                    ? `${results.find((r) => r.commuteKm !== null)!.commuteKm} km`
                    : "Map/area needed"
                }
              />
            </div>
              ...
              {results.map((r) => {
                ...
                return (
                  <Link
                    ...
                    params={{ id: r.pg.id }}
                    key={r.pg.id}
                    className="block rounded-lg border bg-card p-4 hover:border-accent/50"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
                          {r.pg.area} · {r.pg.tier} · {r.pg.gender}
                        </div>
                        <h3 className="font-semibold mt-0.5">{r.pg.name}</h3>
                        <div className="text-xs text-muted-foreground mt-0.5">{r.pg.locality}</div>
                      </div>
                      <div className="text-right">
                        <div className={cn("font-display text-2xl font-semibold", rt.color)}>
                          {r.total}
                        </div>
                        <div
                          className={cn(
