  ...
  head: () => ({
    meta: [
      { title: "Demand heatmap — Gharpayy" },
      {
        name: "description",
        content:
          "Area-level demand vs supply: leads, tours, conversion, vacancy, with strategic insights.",
      },
    ],
  }),
  ...
  const rows = useMemo<AreaRow[]>(() => {
    ...
    return Array.from(areas)
      .map((area) => {
        ...
        const conversion =
          completedTours > 0 ? Math.round((areaBookings / completedTours) * 100) : 0;
        const totalBeds = areaProps.reduce((s, p) => s + p.totalBeds, 0);
        const vacantBeds = areaProps.reduce((s, p) => s + p.vacantBeds, 0);
        const occupancy =
          totalBeds > 0 ? Math.round(((totalBeds - vacantBeds) / totalBeds) * 100) : 0;
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div
                      className={`inline-flex items-center gap-1 text-[11px] font-medium mt-0.5 ${cfg.text}`}
                    >
                ...
                <div className="grid grid-cols-3 gap-2 text-[11px]">
                  ...
                  <Stat
                    label="Avg ₹"
                    value={r.avgBudget ? `${(r.avgBudget / 1000).toFixed(0)}k` : "—"}
                    mono
                  />
                </div>
                ...
                <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                  <div
                    className={`h-full ${r.insight.tone === "hot" ? "bg-destructive" : r.insight.tone === "warm" ? "bg-success" : r.insight.tone === "cold" ? "bg-warning" : "bg-muted-foreground"}`}
                    style={{ width: `${r.pressure}%` }}
                  />
                </div>
  ...
  return (
    <div className="rounded-md bg-background/60 px-2 py-1.5">
      ...
      <div className={`text-xs font-medium ${mono ? "font-mono" : ""}`}>
        {value}
        {hot !== undefined && hot > 0 && (
          <span className="ml-1 text-destructive font-mono text-[10px]">·{hot}🔥</span>
        )}
      </div>
    </div>
  );
