  ...
  head: () => ({
    meta: [
      { title: "Revival queue — Gharpayy" },
      {
        name: "description",
        content:
          "Hidden revenue: cold leads matching new inventory, silent hot leads, resolvable objections.",
      },
    ],
  }),
      ...
      <div className="space-y-6">
        <header>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {candidates.length} hidden opportunit{candidates.length === 1 ? "y" : "ies"} — silent
            leads, cold matches, and now-resolvable objections.
          </p>
        </header>
        ...
        {candidates.length === 0 ? (
          <div className="rounded-xl border border-border bg-card p-12 text-center">
            ...
            <div className="text-xs text-muted-foreground mt-1">
              All leads are active or properly closed.
            </div>
          </div>
        ) : (
                ...
                >
                  <button onClick={() => selectLead(lead.id)} className="flex-1 min-w-0 text-left">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={`inline-flex items-center rounded-md border px-1.5 py-0.5 text-[10px] font-medium ${signalCfg.cls}`}
                      >
                        {signalCfg.label}
                      </span>
                    </div>
