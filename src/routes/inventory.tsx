...
export const Route = createFileRoute("/inventory")({
  head: () => ({
    meta: [
      { title: "Inventory pressure — Gharpayy" },
      {
        name: "description",
        content: "Demand, conversion and pressure scores per property — directs where to push.",
      },
    ],
  }),
      ...
      <div className="space-y-6">
        <header>
          ...
          <p className="text-sm text-muted-foreground">
            Demand vs conversion vs vacancy. Each card tells you what to do next.
          </p>
        </header>
...
function Tile({
  ...
  return (
    <div className="rounded-lg bg-muted/60 px-3 py-2">
      ...
      <div
        className={`font-display text-lg font-semibold tabular-nums ${accent ? "text-accent" : ""}`}
      >
        {value}
        <span className="text-muted-foreground text-xs font-normal">{suffix}</span>
      </div>
    </div>
  );
}
...
function Signal({
  ...
  return (
    <span
      className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-semibold ${cfg.cls}`}
    >
      {cfg.label}
    </span>
  );
...
function Recommendation({
  ...
  return <p className="text-[11px] text-muted-foreground border-t border-border pt-2">{text}</p>;
}
