      ...
      <div className="max-w-3xl space-y-8">
        <header>
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Three roles, three landing pages, one connected machine. Below is the daily rhythm —
            follow it and the system runs itself.
          </p>
        </header>

        <section className="space-y-3">
          <h2 className="font-display text-lg font-semibold flex items-center gap-2">
            <Sun className="h-4 w-4 text-accent" /> Daily operating rhythm
          </h2>
          <ol className="space-y-3">
            <Step
              time="9:30 AM"
              title="Owners open the update window"
              link={{ to: "/owner", label: "Owner Portal" }}
              body="Each owner reviews every room individually. No bulk shortcuts. Status: Occupied / Vacating (date+rent) / Vacant / Blocked."
            />
            <Step
              time="10:30 AM"
              title="Warning bell"
              body="Compliance score starts dropping for owners with unverified rooms."
            />
            <Step
              time="11:00 AM"
              title="Auto-lock"
              link={{ to: "/owner/rooms", label: "View locked rooms" }}
              body="Unverified rooms flip to unsellable and vanish from team's sellable supply."
              accent="danger"
            />
            <Step
              time="11 AM – 1 PM"
              title="Flow Ops activates new rooms"
              link={{ to: "/myt/flow-ops", label: "Flow Ops" }}
              body="Every new room: 5 pitches or 2 qualified matches within 2 hours."
            />
            <Step
              time="1 PM – 7 PM"
              title="TCM runs visits"
              link={{ to: "/myt/tcm", label: "TCM Desk" }}
              body="Each visit tied to a room_id. Post-visit report filed within 15 min — captures objection, budget gap, timeline."
            />
            <Step
              time="Anytime"
              title="Owners approve blocks within 15 min"
              link={{ to: "/owner/blocks", label: "Block requests" }}
              body="High-intent leads need an owner OK in 15 min, else auto-released."
            />
            <Step
              time="7 PM"
              title="HR reviews compliance + leaderboard"
              link={{ to: "/myt", label: "HR Tower" }}
        ...
        <section className="rounded-xl border border-border bg-card p-4 space-y-2">
          <h2 className="font-display text-lg font-semibold flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-info" /> One way to do things
          </h2>
          <ul className="text-sm text-muted-foreground space-y-1.5 list-disc list-inside">
            ...
            <li>
              Every action publishes to the closed-loop event bus → owner sees team activity, team
              sees owner updates.
            </li>
          </ul>
        </section>
...
function Step({
  ...
  return (
    <li
      className={`rounded-lg border p-3 ${accent === "danger" ? "border-destructive/30 bg-destructive/5" : "border-border bg-card"}`}
    >
      <div className="flex items-baseline gap-3 flex-wrap">
        <span
          className={`text-xs font-mono ${accent === "danger" ? "text-destructive" : "text-accent"}`}
        >
          {time}
        </span>
        <span className="font-medium text-sm">{title}</span>
        {link && (
          <Link to={link.to as any} className="text-xs text-accent ml-auto">
            {link.label} →
          </Link>
        )}
      </div>
