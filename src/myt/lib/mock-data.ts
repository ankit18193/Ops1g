...
export const teamMembers: TeamMember[] = names.map((name, i) => {
  ...
  const role =
    posInZone < Math.ceil(membersInZone.length * 0.7) ? ("flow-ops" as const) : ("tcm" as const);
  return {
    id: `m${i + 1}`,
    name,
    role,
...
export const tours: Tour[] = Array.from({ length: 80 }, (_, i) => {
  ...
  return {
    ...
    whyLost:
      outcome === "rejected" ? (["price", "location", "food", "comparing"] as const)[i % 4] : null,
  };
});
...
export const initialLeads: Lead[] = Array.from({ length: 25 }, (_, i) => {
  ...
  const mytQualified =
    areaCovered &&
    budget >= 7000 &&
    (moveIn.getTime() - Date.now()) / (1000 * 60 * 60 * 24) <= 15 &&
    dateConfirmed;
...
export function getMemberPerformance(tourList: Tour[]) {
  return teamMembers.map((member) => {
    ...
    return {
      ...
      toursCompleted: completed.length,
      showUpRate:
        memberTours.length > 0 ? Math.round((showed.length / memberTours.length) * 100) : 0,
      drafts: drafts.length,
      closures: Math.floor(drafts.length * 0.4),
