      ...
      <div className="flex items-center justify-between gap-2 pt-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-muted-foreground">
            {confirmationLabel[t.confirmationStrength]}
          </span>
        </div>
        ...
        {onUpdate && (
          <div className="flex gap-1 items-center">
            ...
            {t.status === "scheduled" && (
              <Button
                size="sm"
                onClick={() => onUpdate(t.id, { status: "confirmed" })}
            ...
            {t.status === "confirmed" && (
              <>
                <Button
                  size="sm"
                  onClick={() => onUpdate(t.id, { status: "completed", showUp: true })}
                  className="h-7 text-[11px] px-2.5"
                >
                  Show
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => onUpdate(t.id, { status: "no-show", showUp: false })}
                  className="h-7 text-[11px] px-2 text-danger border-danger/30"
                >
                  No-Show
                </Button>
              </>
      ...
      {showOutcome && onUpdate && (
        <div className="space-y-2 pt-2 border-t border-border">
          ...
          <div className="flex gap-1.5 flex-wrap">
            <Button
              size="sm"
              onClick={() => setOutcome("booked", { tokenPaid: true })}
              className="h-7 text-[10px] px-2 bg-role-tcm text-background hover:bg-role-tcm/90"
            >
              ✅ Booked
            </Button>
            <Button
              size="sm"
              onClick={() => setOutcome("token-paid", { tokenPaid: true })}
              className="h-7 text-[10px] px-2 bg-role-tcm/80 text-background hover:bg-role-tcm/70"
            >
              💰 Token
            </Button>
            <Button
              ...
              variant="outline"
              className="h-7 text-[10px] px-2 text-role-hr border-role-hr/30"
            >
              📄 Draft
            </Button>
            <Button
            ...
            <Button
              ...
              variant="outline"
              className="h-7 text-[10px] px-2 text-danger border-danger/30"
            >
              ❌ Lost
            </Button>
          </div>
