    ...
    <div className="space-y-4 md:space-y-6 animate-slide-up">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          {currentMemberId
            ? "Sorted by intent — fight for hard ones first"
            : "Select yourself in the header ↑"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-3">
        ...
        <MetricCard
          label="Show-Up %"
          value={myTours.length > 0 ? `${Math.round((showUps / myTours.length) * 100)}%` : "0%"}
          color={showUps / Math.max(1, myTours.length) >= 0.7 ? "green" : "red"}
        />
        <MetricCard
          label="Bookings"
          value={drafts}
      ...
      <div className="glass-card p-3 md:p-4 space-y-3">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Close using the room that matches area, budget and live availability.
            </p>
          </div>
          <span className="text-[10px] rounded-full bg-role-tcm/10 px-2 py-1 text-role-tcm">
            TCM goal: close every Tour
          </span>
        </div>
        <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
          {sortedTours.slice(0, 3).map((tour) => {
            ...
            const prop =
              supplyHubProperties.find((p) => p.name === tour.propertyName) ??
              (fit ? supplyHubProperties.find((p) => p.id === fit.propertyId) : undefined);
            ...
            return (
              <div key={tour.id} className="rounded-lg border border-border bg-surface-2/40 p-3">
                ...
                <div className="text-[11px] text-muted-foreground truncate">
                  {prop?.name ?? tour.propertyName} · {inv?.beds ?? fit?.availableBeds ?? 0} beds
                  live
                </div>
                <div className="mt-2 text-[11px] text-foreground/80">
                  Pitch: {fit?.reason ?? "Use best available room and protect price objection."}
                </div>
                <div className="mt-2 grid grid-cols-2 gap-1 text-[10px] text-muted-foreground">
                  <span>Confirm arrival</span>
                  <span>Show best room</span>
