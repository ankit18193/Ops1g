    ...
    <div className="space-y-4 animate-slide-up">
      ...
      {!isEmpty && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          ...
          <SummaryTile
            label="Missed (7d)"
            value={`₹${(totals.missed / 1000).toFixed(0)}k`}
            accent="danger"
          />
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Field label="Photos uploaded">
            <Input
              type="number"
              value={photoCount}
              onChange={(e) => setPhotoCount(+e.target.value || 0)}
              className="h-9 text-sm"
            />
          </Field>
          <Field label="Food rating (0–5)">
            <Input
              ...
              max={5}
              value={foodRating}
              onChange={(e) => setFoodRating(+e.target.value || 0)}
              className="h-9 text-sm"
            />
          </Field>
          <Field label="Hygiene rating (0–5)">
            <Input
              ...
              max={5}
              value={hygieneRating}
              onChange={(e) => setHygieneRating(+e.target.value || 0)}
              className="h-9 text-sm"
            />
          </Field>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              onClick={() => setRooms((r) => [...r, emptyRoomDraft(basePrice)])}
            >
          ...
          <div className="space-y-2">
            {rooms.map((r, i) => (
              <div
                key={i}
                className="grid grid-cols-12 gap-2 items-end rounded-md border border-border bg-surface-2 p-2"
              >
                <div className="col-span-3">
                  ...
                  <Select
                    ...
                    onValueChange={(v: RoomType) =>
                      setRooms((rs) => rs.map((x, j) => (j === i ? { ...x, type: v } : x)))
                    }
                  >
                ...
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setRooms((rs) =>
                        rs.map((x, j) => (j === i ? { ...x, bedsTotal: +e.target.value || 1 } : x)),
                      )
                    }
                ...
                <div className="col-span-2">
                  ...
                  <Input
                    type="number"
                    min={0}
                    value={r.bedsOccupied}
                    onChange={(e) =>
                      setRooms((rs) =>
                        rs.map((x, j) =>
                          j === i ? { ...x, bedsOccupied: +e.target.value || 0 } : x,
                        ),
                      )
                    }
                    className="h-8 text-xs"
                ...
                <div className="col-span-3">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setRooms((rs) =>
                        rs.map((x, j) =>
                          j === i ? { ...x, currentPrice: +e.target.value || 0 } : x,
                        ),
                      )
                    }
                    className="h-8 text-xs"
        ...
        <div className="sticky top-0 bg-card border-b border-border p-4 flex items-start justify-between z-10">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {property.address} · Owner: {property.ownerName}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 space-y-4">
          <div className="space-y-1.5">
            ...
            {scores.suggestedActions.map((a, i) => (
              <div
                key={i}
                className="text-xs text-foreground bg-primary/5 border border-primary/20 rounded px-3 py-2 flex items-start gap-2"
              >
                <span>💡</span>
                <span>{a}</span>
              </div>
                  ...
                  <div key={r.id} className="rounded-lg border border-border bg-surface-2 p-2.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        ₹{(r.currentPrice / 1000).toFixed(1)}k
                      </span>
                    </div>
                    <div className="text-[10px] text-muted-foreground mt-1">
                      {free} free · {r.bedsOccupied} taken
                ...
                {activeBlocks.map((b) => (
                  ...
                  >
                    <span className="text-foreground">
                      {b.leadName} <span className="text-muted-foreground">· {b.intent}</span>
                    </span>
            ...
            <div>
              ...
              <div className="space-y-1">
                {recentTours.map((t) => (
                  <div
                    key={t.id}
                    className="flex items-center justify-between text-xs bg-surface-2 rounded px-3 py-1.5"
                  >
