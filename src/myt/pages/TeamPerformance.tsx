...
export default function TeamPerformance() {
  ...
  const filtered =
    filterZone === "all" ? memberPerf : memberPerf.filter((m) => m.zoneName.includes(filterZone));
  ...
  const memberTours = selectedMember
    ? tours.filter((t) => t.assignedTo === selectedMember || t.scheduledBy === selectedMember)
    : [];
              ...
              <div>
                ...
                <span
                  className={cn(
                    ...
                    m.role === "flow-ops"
                      ? "bg-flow-ops/15 text-role-flow"
                      : "bg-tcm/15 text-role-tcm",
                  )}
                >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-muted-foreground bg-surface-2/50">
                ...
                <th
                  className="text-center py-3 px-2 font-medium cursor-pointer"
                  onClick={() => handleSort("leadsAdded")}
                >
                  Leads <SortIcon field="leadsAdded" />
                </th>
                <th
                  className="text-center py-3 px-2 font-medium cursor-pointer"
                  onClick={() => handleSort("toursScheduled")}
                >
                  Tours <SortIcon field="toursScheduled" />
                </th>
                <th
                  className="text-center py-3 px-2 font-medium cursor-pointer"
                  onClick={() => handleSort("toursCompleted")}
                >
                  Done <SortIcon field="toursCompleted" />
                </th>
                <th
                  className="text-center py-3 px-2 font-medium cursor-pointer"
                  onClick={() => handleSort("showUpRate")}
                >
                  Show% <SortIcon field="showUpRate" />
                </th>
                <th
                  className="text-center py-3 px-2 font-medium cursor-pointer"
                  onClick={() => handleSort("drafts")}
                >
                  Drafts <SortIcon field="drafts" />
                </th>
                <th
                  className="text-center py-3 px-2 font-medium cursor-pointer"
                  onClick={() => handleSort("sameDayRate")}
                >
                  Same-Day% <SortIcon field="sameDayRate" />
                </th>
              </tr>
            </thead>
            <tbody>
              {sorted.map((m) => (
                <tr
                  ...
                  onClick={() =>
                    setSelectedMember(m.memberId === selectedMember ? null : m.memberId)
                  }
                >
        ...
        <div className="glass-card p-3 md:p-5 animate-slide-up">
          ...
          <div className="space-y-2 md:hidden">
            ...
            {memberTours.length === 0 && (
              <p className="text-center text-muted-foreground text-sm py-4">No tours found</p>
            )}
          </div>
