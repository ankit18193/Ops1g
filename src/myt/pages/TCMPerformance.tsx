...
export default function TCMPerformance() {
  ...
  const rows = [
    ...
    { label: "No-Shows", value: myTours.filter((t) => t.showUp === false).length, danger: true },
    {
      ...
      value: myTours.filter((t) => t.status === "completed" && !t.outcome).length,
      warning: true,
    },
    {
      label: "Draft Conversion",
      value: completed > 0 ? `${Math.round((drafts / completed) * 100)}%` : "0%",
    },
  ];
      ...
      <div className="glass-card p-3 md:p-5">
        ...
        <div className="space-y-0 text-sm">
          {rows.map((r) => (
            <div
              key={r.label}
              className="flex justify-between py-2.5 border-b border-border/50 last:border-0"
            >
              <span className="text-muted-foreground text-xs md:text-sm">{r.label}</span>
              <span
                className={`font-medium text-sm ${r.danger ? "text-danger" : r.warning ? "text-role-hr" : "text-foreground"}`}
              >
                {r.value}
              </span>
            </div>
