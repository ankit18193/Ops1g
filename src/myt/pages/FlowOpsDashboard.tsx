...
export default function FlowOpsDashboard() {
  ...
  const dailyTotals = cycles.reduce(
    (acc, c) => ({
    ...
    <div className="space-y-4 md:space-y-6 animate-slide-up">
      ...
      <div className="flex items-start justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Your scheduling performance · click any tour to open the command panel
          </p>
        </div>
      ...
      <div className="glass-card p-3 md:p-5 space-y-3">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <span className="rounded-full bg-primary/10 px-2 py-1 text-[11px] text-primary">
            {ownedRow.availableBeds} beds · {ownedRow.toursToday} Tours today
          </span>
        </div>
        <div className="grid gap-2 md:grid-cols-2">
          {areaLeads.map((lead) => {
            const fit = bestInventoryFits({
              areaText: lead.area,
              budget: lead.budget,
              rooms,
              blocks,
            ...
            return (
              <div
                key={lead.id}
                className="rounded-lg border border-border bg-surface-2/40 p-3 flex items-center justify-between gap-3"
              >
                <div className="min-w-0">
                  <div className="text-sm font-medium truncate">
                    {lead.name} · ₹{lead.budget.toLocaleString()}
                  </div>
                  <div className="text-[11px] text-muted-foreground truncate">
                    {fit?.propertyName ?? "No live fit"} · TCM {tcm?.name ?? "Auto"}
                  </div>
                </div>
                <Button
                  size="sm"
                  className="h-8 text-xs shrink-0"
                  onClick={() => navigate("/myt/schedule", { state: { lead, inventoryFit: fit } })}
                >
                  Schedule Tour
                </Button>
              </div>
      ...
      <div className="glass-card p-3 md:p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-heading font-semibold text-xs md:text-sm text-foreground">
            90-Min Cycle Tracker
          </h3>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            {
              key: "chatsClosed" as const,
              label: "Chats Closed",
              target: CYCLE_TARGETS.chatsClosed,
            },
            { key: "mytLeads" as const, label: "MYT Leads", target: CYCLE_TARGETS.mytLeads },
            {
              key: "toursScheduled" as const,
              label: "Tours Scheduled",
              target: CYCLE_TARGETS.toursScheduled,
            },
            {
              key: "sameDayConfirmed" as const,
              label: "Same-Day",
              target: CYCLE_TARGETS.sameDayConfirmed,
            },
          ].map((item) => {
              ...
              <div key={item.key} className="bg-surface-2/50 rounded-lg p-3">
                ...
                <div className="flex items-center justify-center gap-2">
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => updateCycle(item.key, -1)}
                    className="h-7 w-7 p-0 text-xs"
                  >
                    −
                  </Button>
                  <span className="text-lg font-heading font-bold text-foreground w-8 text-center">
                    {val}
                  </span>
                  <Button
                    size="sm"
        ...
        <div className="mt-3 pt-3 border-t border-border">
          ...
          <div className="flex gap-3 text-xs">
            <span className="text-foreground">
              <strong>{dailyTotals.chatsClosed}</strong>/120 chats
            </span>
            <span className="text-foreground">
              <strong>{dailyTotals.mytLeads}</strong>/40 MYT
            </span>
            <span className="text-foreground">
              <strong>{dailyTotals.toursScheduled}</strong>/16 tours
            </span>
            <span className="text-foreground">
              <strong>{dailyTotals.sameDayConfirmed}</strong>/8 same-day
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {myTours.length === 0 && (
            <p className="text-xs text-muted-foreground text-center py-4">
              No tours yet — hit "Schedule Tour" above to add one.
            </p>
          )}
          {myTours.map((t) => (
            <div
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full ${t.status === "completed" ? "bg-success/15 text-success" : t.status === "confirmed" ? "bg-tcm/15 text-role-tcm" : "bg-primary/15 text-primary"}`}
                >
                  {t.status}
                </span>
                <LeadControlPanel
