    ...
    <div className="space-y-4 animate-slide-up">
      ...
      <div className="flex items-center gap-3 text-[10px] text-muted-foreground flex-wrap">
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-role-tcm/30 border border-role-tcm" /> Hard
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-role-hr/30 border border-role-hr" /> Medium
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-muted/40 border border-muted-foreground/30" /> Soft
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-surface-2 border border-border" /> Open
        </span>
      </div>
        ...
        <div className="grid" style={{ gridTemplateColumns: "60px repeat(7, minmax(120px, 1fr))" }}>
          ...
          {HOURS.map((h) => (
            <div key={h} className="contents">
              <div className="text-[10px] text-muted-foreground py-1.5 pr-2 text-right border-r border-border">
                {h}
              </div>
              {days.map((d) => {
                ...
                return (
                  <div
                    key={d.iso + h}
                    className="border-b border-r border-border/30 p-1 min-h-[40px] space-y-0.5"
                  >
                    {items.map((t) => (
                      <div
                        key={t.id}
      ...
      {hoveredTour && (
        <div
          className={cn(
            "hidden md:block fixed bottom-4 right-4 z-50 glass-card p-3 max-w-xs border",
            intentBg[hoveredTour.intent],
          )}
        >
          <div className="text-sm font-semibold text-foreground">{hoveredTour.leadName}</div>
          <div className="text-xs text-muted-foreground">
            {hoveredTour.propertyName} · {hoveredTour.tourTime}
          </div>
          <div className="text-xs mt-1">
            {hoveredTour.confidenceScore}% · {hoveredTour.confidenceReason.join(" · ")}
          </div>
        </div>
      )}
              ...
              {dayTours.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {dayTours.map((t) => (
                    ...
                    >
                      <span className="text-[10px] font-mono text-foreground tabular-nums w-12">
                        {t.tourTime}
                      </span>
                      {tourIcon(t.tourType)}
