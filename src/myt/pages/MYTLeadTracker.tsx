...
export default function MYTLeadTracker() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast.success(
      mytQualified ? "MYT Qualified! Ready for tour" : "Lead added — not MYT qualified",
    );
    setForm({
      name: "",
      phone: "",
    ...
    <div className="space-y-4 animate-slide-up">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            title={
              pipSupported
                ? "Pop dashboard out as a floating window over WhatsApp"
          ...
          <Button
            ...
            variant="default"
            onClick={async () => {
              if (pipSupported) await openPip("capture");
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-3 text-xs flex items-start gap-2">
          ...
          <div>
            ...
            <div className="text-muted-foreground">
              ...
              <strong>Chrome, Edge, Brave or Opera</strong> on desktop. Alternative: use
              split-screen (Windows: <kbd className="px-1 rounded border border-border">Win</kbd>+
              <kbd className="px-1 rounded border border-border">←</kbd> · macOS: drag tab into a
              Stage Manager group).
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-1 rounded-lg border border-border p-0.5 bg-surface-2/50 w-fit">
        <button
          ...
          className={`px-2.5 py-1 text-[11px] rounded-md ${mode === "quick" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
        ...
        <button
          ...
          className={`px-2.5 py-1 text-[11px] rounded-md ${mode === "manual" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
          Manual
        </button>
        <button
          onClick={() => setMode("requests")}
          className={`px-2.5 py-1 text-[11px] rounded-md ${mode === "requests" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
          Requests
        </button>
      </div>
      ...
      {mode === "quick" && (
        <div className="glass-card p-4 space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              All pasted leads now go through Quick Add questions only, so paste/manual/dedup data
              stays one object.
            </p>
          </div>
      ...
      <div className="glass-card p-3 md:p-5">
        <h3 className="font-heading font-semibold text-xs md:text-sm mb-3 text-role-tcm">
          ✅ MYT Qualified — Push to Tour
        </h3>
        <div className="space-y-2">
          {qualified.length === 0 && (
            <p className="text-xs text-muted-foreground text-center py-4">No qualified leads yet</p>
          )}
          {qualified.map((l) => (
            <div
              key={l.id}
              className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-3 py-2.5 rounded-lg bg-surface-2/50"
            >
              <div className="min-w-0">
                <span className="font-medium text-foreground text-sm">{l.name}</span>
                <span className="text-muted-foreground text-xs ml-2">
                  {l.area} · ₹{l.budget.toLocaleString()}
                </span>
              ...
              <div className="flex gap-2 shrink-0">
                ...
                {l.status === "tour-scheduled" && (
                  <span className="text-[10px] px-2 py-1 rounded-full bg-role-tcm/15 text-role-tcm">
                    Tour Set
                  </span>
                )}
              </div>
      ...
      {/* Unqualified */}
      <div className="glass-card p-3 md:p-5">
        <h3 className="font-heading font-semibold text-xs md:text-sm mb-3 text-danger">
          ❌ Not Qualified
        </h3>
        <div className="space-y-2">
          ...
          {unqualified.map((l) => (
            <div
              key={l.id}
              className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-3 py-2.5 rounded-lg bg-surface-2/30"
            >
              <div className="min-w-0">
                <span className="font-medium text-foreground text-sm">{l.name}</span>
                <span className="text-muted-foreground text-xs ml-2">
