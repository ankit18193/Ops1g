...
export default function ScheduleTour({ onScheduled }: ScheduleTourProps = {}) {
  ...
  const routeState = useMemo(
    () =>
      location.state as {
        ...
        pastedLead?: Record<string, unknown>;
  ...
  useEffect(() => {
    const stateLead = incomingLead || pastedLead;
    if (!stateLead) return;
    const leadName = String(stateLead.name ?? pastedLead?.name ?? "");
    const phone = String(
      stateLead.phone ?? stateLead.phoneRaw ?? stateLead.phoneE164 ?? pastedLead?.phone ?? "",
    );
    ...
    const area = String(
      pastedLead?.area ??
        pastedLead?.location ??
        pastedLead?.fullAddress ??
        stateLead.area ??
        stateLead.location ??
        stateLead.fullAddress ??
        "",
    );
    const room = String(
      pastedLead?.room ?? stateLead.room ?? stateLead.roomType ?? "Single",
    ).toLowerCase();
    const type = String(pastedLead?.type ?? stateLead.type ?? "");
    const notes = String(
      pastedLead?.specialReqs ??
        pastedLead?.extraContent ??
        stateLead.notes ??
        stateLead.extraContent ??
        "",
    );
    setForm((f) => ({
      ...f,
  ...
  const canSubmit =
    form.leadName && form.phone && form.propertyName && form.tourTime && effectiveTcm;
  ...
  const handleSubmit = () => {
    ...
    const matchingProp = supplyHubProperties.find(
    ...
    if (matchingProp) {
      ...
      if (block) {
        ...
        toast.success(
          `${intent.toUpperCase()} tour → ${effectiveTcm.name} · Room held ${intent === "hard" ? "4h" : "1h"}`,
        );
      } else {
  ...
  const debugAreaText = String(
    pastedLead?.area ??
      pastedLead?.location ??
      pastedLead?.fullAddress ??
      incomingLead?.area ??
      incomingLead?.location ??
      form.workLocation ??
      "",
  );
  const detectedArea = debugAreaText
    ? detectAreaZone(debugAreaText)
  ...
  const selectedProperty = supplyHubProperties.find(
    (p) => p.name === form.propertyName || p.id === incomingFit?.propertyId,
  );
  const selectedInventory = selectedProperty
    ? availableBedsForProperty(selectedProperty.id, rooms, blocks)
    : null;
  const calculatedFits = useMemo(
    () =>
      debugAreaText
        ? bestInventoryFits({
            areaText: debugAreaText,
            budget: debugBudget,
            room: String(pastedLead?.room ?? incomingLead?.room ?? form.roomType),
            rooms,
            blocks,
            limit: 3,
          })
  ...
  const debugLinks = [
    ...
    ...(String(pastedLead?.fullAddress ?? incomingLead?.fullAddress ?? "").match(
      /https?:\/\/\S+/g,
    ) ?? []),
  ];
  const unifiedFieldValues: Record<string, string> = {
    ...
    areas: String(
      pastedLead?.area ?? pastedLead?.areas ?? incomingLead?.area ?? form.workLocation ?? "",
    ),
  ...
  const missingUnifiedFields = QUICKAD_QUESTIONS.filter(
    (q) => !String(unifiedFieldValues[q.key] ?? "").trim(),
  );
    ...
    <div className="space-y-4 animate-slide-up max-w-3xl">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className={cn("rounded-xl border p-3 min-w-[200px]", intentBg[intent])}>
          ...
          {reason.length > 0 && (
            <p className="text-[10px] mt-1.5 leading-snug opacity-80">{reason.join(" · ")}</p>
          )}
        </div>
      </div>
        ...
        <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="text-sm font-semibold text-foreground">
              {(location.state as { inventoryFit?: InventoryFit }).inventoryFit?.propertyName} ·{" "}
              {(location.state as { inventoryFit?: InventoryFit }).inventoryFit?.availableBeds} beds
              live
            </div>
          </div>
          <div className="text-[11px] text-muted-foreground">
            {(location.state as { inventoryFit?: InventoryFit }).inventoryFit?.reason}
          </div>
        </div>
      )}
      ...
      <div className="rounded-lg border border-border bg-surface-2/60 p-3 space-y-3">
        <div className="flex items-center justify-between gap-2">
          ...
          <Badge variant="secondary" className="text-[10px]">
            {incomingFit ? "Quick Add match" : "Live match"}
          </Badge>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px]">
          ...
          <DebugMetric
            label="Budget"
            value={debugBudget ? `₹${debugBudget.toLocaleString("en-IN")}` : "—"}
          />
          <DebugMetric
            label="Available beds"
            value={String(
              incomingFit?.availableBeds ??
                selectedInventory?.beds ??
                calculatedFits[0]?.availableBeds ??
                0,
            )}
          />
        </div>
        <div className="grid gap-1.5">
          {(calculatedFits.length ? calculatedFits : incomingFit ? [incomingFit] : [])
            .slice(0, 3)
            .map((fit) => (
              <div
                key={fit.propertyId}
                className="flex items-center justify-between gap-2 rounded-md border border-border bg-background/70 px-2 py-1.5 text-[11px]"
              >
                <span className="truncate font-medium text-foreground">{fit.propertyName}</span>
                <span className="shrink-0 text-muted-foreground">
                  {fit.availableBeds} beds · {fit.priceFit} · score {fit.score}
                </span>
              </div>
            ))}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-muted-foreground">
          <div className="flex items-start gap-1.5">
            <MapPin className="h-3 w-3 mt-0.5 text-primary" /> From here:{" "}
            {selectedProperty
              ? `${selectedProperty.address} → ${debugAreaText || "lead location"}`
              : "select property to compare"}
          </div>
          <div className="flex items-start gap-1.5">
            <MapPin className="h-3 w-3 mt-0.5 text-primary" /> From there:{" "}
            {debugLinks[0]
              ? `map link captured (${debugLinks.length})`
              : "no map link captured yet"}
          </div>
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between gap-2">
          ...
          <Badge variant="secondary" className="text-[10px]">
            {QUICKAD_QUESTIONS.length - missingUnifiedFields.length}/{QUICKAD_QUESTIONS.length}{" "}
            synced
          </Badge>
        </div>
        ...
        {missingUnifiedFields.length > 0 && (
          <p className="text-[11px] text-muted-foreground">
            Missing: {missingUnifiedFields.map((q) => q.label).join(", ")}. Existing duplicate
            fields stay prefilled; only these gaps need attention.
          </p>
        )}
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, bookingSource: e.target.value as BookingSource }))
                }
                className={select}
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, decisionMaker: e.target.value as DecisionMaker }))
                }
                className={select}
              >
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    kind === "positive" ? "text-role-tcm" : "text-amber-foreground",
                  )}
                >
                  {kind === "positive" ? "+" : "−"}
                </span>
        ...
        <div className="glass-card p-4 md:p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                value={form.zoneId}
                onChange={(e) => setForm((f) => ({ ...f, zoneId: e.target.value, assignedTo: "" }))}
                className={select}
              >
                {zones.map((z) => (
            ...
            <div>
              ...
              >
                ...
                {supplyHubProperties
                  .filter((p) => p.zoneId === form.zoneId)
                  .map((p) => (
                    <option key={p.id} value={p.name}>
                      {p.name} · ₹{(p.basePrice / 1000).toFixed(0)}k
                    </option>
                  ))}
              </select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                ...
                max={in7days()}
                value={form.tourDate}
                onChange={(e) => setForm((f) => ({ ...f, tourDate: e.target.value, tourTime: "" }))}
                className="bg-surface-2 border-border"
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, assignedTo: e.target.value, tourTime: "" }))
                }
                className={select}
              >
          ...
          <div>
            <Label className={labelCls}>
              Pick Slot{" "}
              {effectiveTcm && (
                <span className="ml-2 normal-case text-foreground/60">({effectiveTcm.name})</span>
              )}
            </Label>
