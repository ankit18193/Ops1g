...
export default function Bookings() {
  ...
  const signed = bookings.filter(
    (b) => b.agreementStatus === "signed" || b.agreementStatus === "moved-in",
  ).length;
        ...
        <form onSubmit={handleSubmit} className="glass-card p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select
                value={form.closedBy}
                onChange={(e) => setForm((f) => ({ ...f, closedBy: e.target.value }))}
                className={selectClass}
              >
            ...
            <label className="flex items-center gap-2 text-xs text-muted-foreground self-end pb-2">
              <input
                type="checkbox"
                checked={form.viaTour}
                onChange={(e) => setForm((f) => ({ ...f, viaTour: e.target.checked }))}
                className="rounded"
              />
              Via Tour (not direct)
            </label>
        ...
        {bookings.map((b) => (
          <div key={b.id} className="glass-card p-3 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-heading font-bold text-role-tcm">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <span
                className={cn(
                  ...
                  statusColors[b.agreementStatus],
                )}
              >
                {b.agreementStatus}
              </span>
              <span className="text-muted-foreground">
                {b.viaTour ? "🏠 Via Tour" : "📞 Direct"}
              </span>
              ...
              <div className="flex gap-2">
                {b.agreementStatus === "pending" && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => updateStatus(b.id, "signed")}
                ...
                {b.agreementStatus === "signed" && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => updateStatus(b.id, "moved-in")}
                    className="h-7 text-[10px]"
                  >
                    Mark Moved In
                  </Button>
