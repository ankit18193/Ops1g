...
export default function Leaderboard() {
    ...
    .map((m) => {
      const score =
        Math.round(
          m.toursScheduled *
            (m.showUpRate / 100) *
            (m.drafts > 0 ? m.drafts / m.toursCompleted : 0) *
            100,
        ) / 100;
      return { ...m, score };
    })
  ...
  const selectClass =
    "h-8 bg-surface-2 border border-border rounded-md px-2 text-xs text-foreground";
  const rankIcons = [
    ...
    <div className="space-y-4 animate-slide-up">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {zones.map((z) => (
              <option key={z.id} value={z.name.split(" — ")[1]}>
                {z.name.split(" — ")[1]}
              </option>
            ))}
          </select>
      ...
      <div className="glass-card p-3 md:p-5">
        <div className="space-y-0">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground">
                  {m.zoneName.split(" — ")[1]} · {m.role === "tcm" ? "TCM" : "Flow Ops"}
                </p>
              </div>
          ...
          {scored.length === 0 && (
            <p className="text-center text-muted-foreground text-xs py-6">No data</p>
          )}
        </div>
      </div>
    </div>
