...
export default function LeadMarketplace() {
  ...
  const enriched: Enriched[] = useMemo(() => {
    return leads
      ...
      .filter(
        (l) => !globalZoneFilter || zones.find((z) => z.id === globalZoneFilter)?.area === l.area,
      )
      .map((l) => {
  ...
  const claimLead = (leadId: string) => {
    ...
    setLeads((prev) =>
      prev.map((l) =>
        l.id === leadId ? { ...l, claimedBy: currentMemberId, status: "qualified" } : l,
      ),
    );
  ...
  const summary = {
    ...
    avgProb: enriched.length
      ? Math.round(enriched.reduce((s, e) => s + e.conversionProb, 0) / enriched.length)
      : 0,
  };
      ...
      <div className="space-y-2">
        {enriched.length === 0 && (
          <div className="glass-card p-8 text-center text-sm text-muted-foreground">
            No live leads right now. New ones surface as Flow Ops adds them.
          </div>
        )}
        {enriched.map((e) => (
          <div
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 text-[11px] text-muted-foreground mt-0.5 flex-wrap">
                  ...
                  <span className="flex items-center gap-1">
                    <Wallet className="h-3 w-3" />₹{(e.lead.budget / 1000).toFixed(0)}k
                  </span>
                  <span className="flex items-center gap-1">
                    <Calendar className="h-3 w-3" />
                    Move {e.lead.moveInDate.slice(5)}
                  </span>
                </div>
              </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              {currentRole === "tcm" && !e.lead.claimedBy && (
                <Button
                  size="sm"
                  onClick={() => claimLead(e.lead.id)}
                  className="h-8 text-xs flex-1"
                >
                  <Hand className="h-3 w-3 mr-1" /> Claim
                </Button>
              )}
              {(currentRole === "flow-ops" ||
                (currentRole === "tcm" && e.lead.claimedBy === currentMemberId)) && (
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => scheduleFromLead(e.lead)}
                  className="h-8 text-xs flex-1"
                >
                  Schedule tour →
                </Button>
