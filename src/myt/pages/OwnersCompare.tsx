  ...
  const data = useMemo(() => {
    return owners
      .map((o) => {
        ...
        const myRoomsCount =
          myStatuses.length ||
          rooms.filter((r) => myProps.some((p) => p.id === r.propertyId)).length;
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <SummaryCard
          label="Top performer"
          value={best.owner.name}
          sub={`Trust ${best.compliance.score} · ${best.verifyRate}% verified`}
          tone="success"
        />
        <SummaryCard
          label="Needs help"
          value={worst.owner.name}
          sub={`Trust ${worst.compliance.score} · ${worst.lockRate}% locked`}
          tone="destructive"
        />
        <SummaryCard
          label="Open requests"
            ...
            <tbody>
              {data.map((row) => (
                ...
                >
                    ...
                    <div className="inline-flex items-center gap-1.5">
                      <span
                        className={cn(
                          ...
                          row.compliance.score >= 90
                            ? "text-success"
                            : row.compliance.score >= 70
                              ? "text-info"
                              : "text-destructive",
                        )}
                      >
                  ...
                  <td className="px-3 py-2.5 text-right">
                    <span className={cn("font-mono", row.lockRate > 30 ? "text-destructive" : "")}>
                      {row.locked}
                    </span>
          ...
          <div key={row.owner.id} className="rounded-lg border border-border bg-card p-4 space-y-2">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-[11px] text-muted-foreground">
                  {row.myRoomsCount} rooms · {row.owner.propertyIds.length} property
                </div>
              </div>
              <TierPill tier={row.compliance.tier} />
            </div>
            <div className="grid grid-cols-3 gap-2 text-center">
              <Stat
                ...
                label="Trust"
                value={String(row.compliance.score)}
              />
              <Stat
                icon={<Camera className="h-3 w-3" />}
                label="Verified"
                value={`${row.verifyRate}%`}
              />
              <Stat
                icon={<Lock className="h-3 w-3" />}
                label="Locked"
                value={`${row.lockRate}%`}
                tone={row.lockRate > 30 ? "destructive" : undefined}
              />
            </div>
            {row.pendingBlocks > 0 && (
              <div className="text-[11px] text-warning inline-flex items-center gap-1">
                <Inbox className="h-3 w-3" /> {row.pendingBlocks} block request
                {row.pendingBlocks > 1 ? "s" : ""} awaiting decision
              </div>
            )}
          </div>
        ))}
...
function TierPill({ tier }: { tier: "priority" | "standard" | "throttled" }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-wider font-semibold",
        map[tier],
      )}
    >
...
function Stat({
  ...
  return (
    <div className="rounded-md border border-border bg-background/60 p-2">
      ...
      <div
        className={cn(
          "font-display font-semibold text-sm mt-0.5",
