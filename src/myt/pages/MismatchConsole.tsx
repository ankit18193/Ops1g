...
export default function MismatchConsole() {
  ...
  const flagged = useMemo(() => {
    ...
    tours.forEach((t) => {
      ...
      if (ms.length) {
        const sev = ms.some((m) => m.severity === "high")
          ? "high"
          : ms.some((m) => m.severity === "med")
            ? "med"
            : "low";
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Flagged tours</div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold text-destructive">
              {flagged.filter((f) => f.sev === "high").length}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">TCMs with gaps</div>
            <div className="text-2xl font-bold">{tcmStats.filter((t) => t.flagged > 0).length}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">TCM inconsistency ranking</CardTitle>
        </CardHeader>
        <CardContent>
          {tcmStats.length === 0 ? (
              ...
              {tcmStats.map((t) => (
                <div key={t.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 bg-muted rounded overflow-hidden">
                    <div
                      className={`h-full ${t.pct > 30 ? "bg-destructive" : t.pct > 10 ? "bg-amber-500" : "bg-emerald-500"}`}
                      style={{ width: `${t.pct}%` }}
                    />
                  </div>
                  <div className="text-xs tabular-nums w-24 text-right">
                    {t.flagged}/{t.total} ({t.pct}%)
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <AlertTriangle className="h-4 w-4 text-destructive" /> Flagged tours
          </CardTitle>
        </CardHeader>
        <CardContent>
          {flagged.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No mismatches detected — TCM reports and customer feedback are aligned.
            </p>
          ) : (
            <div className="space-y-2">
              {flagged.map(({ tour, reasons, sev }) => (
                <Link
                  key={tour.id}
                  to={`/myt/tour/${tour.id}`}
                  className="block border rounded p-2 hover:border-primary transition-colors"
                >
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div>
                      <div className="text-sm font-medium">
                        {tour.leadName} · {tour.propertyName}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {tour.assignedToName} · {tour.area}
                      </div>
                    </div>
