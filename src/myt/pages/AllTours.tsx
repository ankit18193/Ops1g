    ...
    <div className="space-y-4 md:space-y-6 animate-slide-up">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <select
          ...
          onChange={(e) =>
            setOutcomeFilter(e.target.value === "all" ? "all" : (e.target.value as TourOutcome))
          }
          className="bg-surface-2 border border-border rounded-lg px-2 py-1.5 text-xs text-foreground"
        >
        ...
        {filtered.slice(0, 30).map((t) => (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground">
              {t.propertyName} · {t.area}
            </p>
            <p className="text-[10px] text-muted-foreground">
              TCM: {t.assignedToName} · By: {t.scheduledByName}
            </p>
