...
export const seedRoomStatuses: OwnerRoomStatus[] = (mytRooms || []).map((r, idx) => {
  ...
  const kind: OwnerRoomStatus["kind"] =
    free === 0 ? "occupied" : free === r.bedsTotal ? "vacant" : "occupied";
...
export const seedObjections: OwnerObjection[] = seedRoomStatuses.slice(0, 8).map((r, i) => ({
  ...
  ownerId: r.ownerId,
  reason: (
    ["price", "location", "price", "amenities", "price", "timing", "location", "price"] as const
  )[i],
...
export const seedInsights: OwnerInsightDaily[] = seedOwners.map((o) => ({
  ...
  topObjection: ["Price ₹1.5k high", "Wants AC", "Far from metro", "Food not preferred"][
