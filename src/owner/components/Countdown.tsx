...
export function Countdown({
  ...
  if (diff <= 0)
    return <span className="font-mono tabular-nums text-destructive">{prefix}expired</span>;
  ...
  return (
    <span className={`font-mono tabular-nums ${urgent ? "text-destructive font-medium" : ""}`}>
      {prefix}
      {txt}
    </span>
  );
