...
interface OwnerCtxValue {
  ...
  updateRoomStatus: (
    roomId: string,
    patch: Partial<Omit<OwnerRoomStatus, "roomId" | "propertyId" | "ownerId">>,
  ) => void;
  ...
  requestBlock: (
    input: Omit<OwnerBlockRequest, "id" | "requestedAt" | "expiresAt" | "state">,
  ) => void;
...
export function OwnerProvider({ children }: { children: React.ReactNode }) {
  ...
  const [currentOwnerId, setCurrentOwnerId] = useState<string | null>(
    persisted.currentOwnerId ?? "own-1",
  );
  ...
  const [roomStatuses, setRoomStatuses] = useState<OwnerRoomStatus[]>(
    persisted.roomStatuses ?? seedRoomStatuses,
  );
  ...
  const [objections, setObjections] = useState<OwnerObjection[]>(
    persisted.objections ?? seedObjections,
  );
        ...
        setRoomStatuses((prev) => {
          ...
          const next = prev.map((r) => {
            if (!r.verifiedToday && !r.lockedUnsellable) {
              changed = true;
              glueBus.publish({
                type: "owner.room.locked",
                roomId: r.roomId,
                propertyId: r.propertyId,
                ownerId: r.ownerId,
                reason: "unverified_by_11am",
              });
    ...
    setRoomStatuses((prev) =>
      prev.map((r) =>
        set.has(r.roomId)
          ? {
              ...r,
              rentConfirmed: Math.max(0, (r.rentConfirmed ?? 0) + delta),
              updatedAt: new Date().toISOString(),
              verifiedToday: true,
            }
          : r,
  ...
  const addRoom: OwnerCtxValue["addRoom"] = ({
    propertyId,
    type,
    bedsTotal,
    price,
  ...
  const logObjection: OwnerCtxValue["logObjection"] = ({
    roomId,
    reason,
    notes,
    loggedBy = "Sales",
  ...
  const bumpRoomViews = (roomId: string, by = 1) => {
    setRoomStatuses((prev) =>
      prev.map((r) => (r.roomId === roomId ? { ...r, views: (r.views ?? 0) + by } : r)),
    );
  };
