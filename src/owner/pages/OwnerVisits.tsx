...
export function OwnerVisits() {
  ...
  const events = useGlueEvents(
    (e) =>
      (e.type === "team.visit.scheduled" ||
        e.type === "team.visit.started" ||
        e.type === "team.visit.ended" ||
        e.type === "tour.confirmation.sent" ||
        e.type === "team.lead.pitched") &&
      (!("ownerId" in e) || !e.ownerId || e.ownerId === currentOwnerId),
    30,
  );
    ...
    <div className="space-y-5 pb-12">
      <header>
        <h1 className="font-display text-xl md:text-2xl font-semibold tracking-tight">
          Tours at your property
        </h1>
        <p className="text-sm text-muted-foreground">
          Live feed from Flow Ops + TCM team — no separate app needed.
        </p>
      </header>
            ...
            {todays.map((t) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn(
                      "h-10 w-10 rounded-xl grid place-items-center text-white font-mono font-bold text-[10px] shadow-sm",
                      roomHeroClass(t.id),
                    )}
                  >
                  ...
                  <div className="text-[11px] font-mono text-muted-foreground">
                    {format(new Date(t.scheduledAt), "p")} · {t.status}
                  </div>
                </div>
              );
            })}
              ...
              return (
                <div key={i} className="flex items-center gap-3 py-2 text-xs">
                  ...
                  <span className="font-mono text-muted-foreground text-[10px] truncate">
                    {("tourId" in e && (e as any).tourId) ||
                      ("leadId" in e && (e as any).leadId) ||
                      ""}
                  </span>
                </div>
              );
