...
export function OwnerRooms() {
  ...
  const [bulkAction, setBulkAction] = useState<"verify" | "rent_down_500" | "rent_up_500">(
    "verify",
  );
    ...
    <div className="space-y-5 pb-32">
      <header className="flex items-end justify-between flex-wrap gap-2">
        <div>
          <h1 className="font-display text-xl md:text-2xl font-semibold tracking-tight">
            Update rooms · per-room precision
          </h1>
          <p className="text-sm text-muted-foreground">
            No bulk shortcuts. Each room confirmed individually before 11 AM.
          </p>
        </div>
        <div className="hidden md:flex gap-2">
          ...
          <Button
            size="sm"
            onClick={() => setAddRoomFor(myProps[0]?.id ?? null)}
            disabled={!myProps.length}
          >
      ...
      {truth.phase === "locked" && (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-xs flex items-center gap-2">
          <Lock className="h-3.5 w-3.5 text-destructive" /> Window closed. Updates still allowed but
          rooms missed today's truth check.
        </div>
      )}
      ...
      {myProps.map((property) => {
        ...
        const sellable = propStatuses.filter(
          (s) =>
            s.verifiedToday &&
            !s.lockedUnsellable &&
            (s.kind === "vacant" || s.kind === "vacating"),
        ).length;
        return (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {propStatuses.map((s) => {
                ...
                return (
                  <div
                    key={s.roomId}
                    className={cn("rounded-xl border bg-card overflow-hidden flex flex-col", tone)}
                  >
                    {/* Hero strip */}
                    <div
                      className={cn("h-20 relative flex items-end p-3", roomHeroClass(s.roomId))}
                    >
                      {s.isDedicated && (
                      ...
                      <div className="text-white">
                        <div className="font-mono text-[9px] opacity-80 tracking-widest uppercase">
                          Room
                        </div>
                        <div className="text-lg font-display font-semibold leading-none capitalize">
                          {r?.type ?? "—"}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-3 space-y-2 flex-1 flex flex-col">
                      <div className="flex items-baseline gap-2">
                        ...
                        {s.floorPrice && (
                          <span
                            className="ml-auto text-[10px] text-muted-foreground inline-flex items-center gap-0.5"
                            title="Your floor price (private)"
                          >
                            <IndianRupee className="h-2.5 w-2.5" />
                            floor {s.floorPrice.toLocaleString()}
                          </span>
                        )}
                      </div>
                      ...
                      {s.lockedUnsellable && (
                        <div className="text-[11px] text-destructive font-medium">
                          ⚠ Auto-locked — not verified by 11 AM
                        </div>
                      )}

                      <div className="text-[10px] text-muted-foreground inline-flex items-center gap-2">
                        {s.verifiedToday && !s.lockedUnsellable && (
                          <span className="text-success inline-flex items-center gap-1">
                            <Check className="h-3 w-3" /> verified today
                          </span>
                        )}
                        {!s.verifiedToday && !s.lockedUnsellable && (
                      ...
                      <div className="mt-auto pt-1 flex items-center gap-2">
                        <div className="flex items-center gap-1.5 flex-1">
                          <Switch
                            ...
                            onCheckedChange={() => {
                              ...
                              toast(
                                s.isDedicated
                                  ? "Removed from dedicated"
                                  : "Added to dedicated supply",
                              );
                            }}
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => markRoomVerified(s.roomId)}
                        >
                          No change
                        </Button>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This is the source of truth. Sales sees this in seconds.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-1">
            <div className="space-y-1.5">
              ...
              <Select
                value={editForm.kind}
                onValueChange={(v) => setEditForm((f) => ({ ...f, kind: v as RoomStatusKind }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1.5">
                ...
                <Input
                  type="number"
                  value={editForm.rentConfirmed}
                  onChange={(e) => setEditForm((f) => ({ ...f, rentConfirmed: e.target.value }))}
                />
              </div>
              <div className="space-y-1.5">
                <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-mono">
                ...
                <Input
                  ...
                  onChange={(e) => setEditForm((f) => ({ ...f, floorPrice: e.target.value }))}
                />
              </div>
            </div>
            {editForm.kind === "vacating" && (
              <div className="space-y-1.5">
                ...
                <Input
                  type="date"
                  value={editForm.vacatingDate}
                  onChange={(e) => setEditForm((f) => ({ ...f, vacatingDate: e.target.value }))}
                />
              </div>
            )}
            <div className="space-y-1.5">
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                value={propForm.area}
                onChange={(e) => setPropForm((f) => ({ ...f, area: e.target.value }))}
                placeholder="Koramangala 5th Block"
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setAddPropOpen(false)}>
              Cancel
            </Button>
      ...
      <Dialog open={!!addRoomFor} onOpenChange={(o) => !o && setAddRoomFor(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {properties.find((p) => p.id === addRoomFor)?.name}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1.5">
                ...
                <Select
                  value={roomForm.type}
                  onValueChange={(v) => setRoomForm((f) => ({ ...f, type: v as any }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  type="number"
                  value={roomForm.bedsTotal}
                  onChange={(e) => setRoomForm((f) => ({ ...f, bedsTotal: e.target.value }))}
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  type="number"
                  value={roomForm.floorPrice}
                  onChange={(e) => setRoomForm((f) => ({ ...f, floorPrice: e.target.value }))}
                />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setAddRoomFor(null)}>
              Cancel
            </Button>
      ...
      <Dialog open={bulkOpen} onOpenChange={setBulkOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Apply the same change to many rooms — emergency lever only.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(v) => {
                            setBulkSelected((prev) =>
                              v ? [...prev, s.roomId] : prev.filter((x) => x !== s.roomId),
                            );
                          }}
                        />
                        <span className="flex-1 truncate">
                          {p?.name} · {r?.type} ({r?.bedsTotal}b)
                        </span>
