    ...
    <div className="space-y-4">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          High-intent leads waiting for your approval. Auto-released after 15 min.
        </p>
      </header>
        ...
        <div className="space-y-2">
          {pending.map((b) => {
            ...
            return (
              <div
                key={b.id}
                className={`rounded-xl border p-3 ${urgent ? "border-destructive/40 bg-destructive/5" : "border-border bg-card"}`}
              >
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="min-w-0">
                    <div className="text-sm font-medium">{b.leadName}</div>
                    <div className="text-[11px] text-muted-foreground">
                      Room {b.roomId} · intent{" "}
                      <span className="font-medium capitalize">{b.intent}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-xs font-mono inline-flex items-center gap-1 ${urgent ? "text-destructive" : "text-muted-foreground"}`}
                    >
        ...
        <section>
          ...
          <div className="space-y-1">
            {decided.slice(0, 8).map((b) => (
              ...
              >
                <span>
                  {b.leadName} · room {b.roomId}
                </span>
                <span
                  className={`font-medium ${b.state === "approved" ? "text-success" : b.state === "rejected" ? "text-destructive" : "text-muted-foreground"}`}
                >
                  {b.state.replace("_", " ")}
                </span>
              </div>
